  ...
  async notifyJobMatch(applicantId, jobTitle, companyName, jobId) {
    try {
      ...
      if (settings.emailEnabled && settings.emailJobMatches && email) {
        const subject = `New E-GovEp Match: ${jobTitle} at ${companyName}`;
        const htmlContent = `
          <h2>🎯 New E-GovEp Match Found!</h2>
          ...
          <p>Best regards,<br>E-GovEp Team</p>
        `;
      ...
      if (settings.smsEnabled && phone) {
        const message = `E-GovEp Alert: ${jobTitle} at ${companyName}. View details at ${process.env.FRONTEND_URL}/jobs/${jobId}`;
